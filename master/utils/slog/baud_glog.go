...
func (s *severity) Set(logging *loggingT, value string) error {
...
// Set is part of the flag.Value interface.
func (l *Level) Set(logging *loggingT, value string) error {
	v, err := strconv.Atoi(value)
	if err != nil {
		return err
	}
...
func (m *moduleSpec) String(logging *loggingT) string {
...
func (m *moduleSpec) Set(logging *loggingT, value string) error {
...
func (t *traceLocation) String(logging *loggingT) string {
	// Lock because the type is not atomic. TODO: clean this up.
	logging.mu.Lock()
	defer logging.mu.Unlock()
...
func (t *traceLocation) Set(logging *loggingT, value string) error {
	if value == "" {
		// Unset.
		t.line = 0
		t.file = ""
...
func (l *loggingT) output(s severity, buf *buffer, file string, line int) {
	...
	if l.toStderr {
		...
		case errorLog:
			...
			//fallthrough
		case warningLog:
			if l.outputLevel > warningLog {
				break
			}
			l.file[warningLog].Write(data)
			//fallthrough
		case infoLog:
			if l.outputLevel > infoLog {
				break
			}
...
// a lock.
func timeoutFlush(logging *loggingT, timeout time.Duration) {
	done := make(chan bool, 1)
	go func() {
		logging.lockAndFlushAll() // calls logging.lockAndFlushAll()
		done <- true
	}()
	select {
	case <-done:
...
func (sb *syncBuffer) Write(p []byte) (n int, err error) {
	if sb.nbytes+uint64(len(p)) >= sb.logger.rotateSize {
		if err := sb.rotateFile(sb.logger, time.Now()); err != nil {
			sb.logger.exit(err)
		}
	}
...
func (sb *syncBuffer) rotateFile(logging *loggingT, now time.Time) error {
	if sb.file != nil {
		sb.Flush()
		sb.file.Close()
	}
	var err error
	sb.file, err = renameAndCreate(logging.dir, logging.module, severityName[sb.sev], now)
	sb.nbytes = 0
	if err != nil {
		return err
	}
...
func (l *loggingT) createFiles(sev severity) error {
	...
	for s := sev; s >= l.outputLevel && l.file[s] == nil; s-- {
		sb := &syncBuffer{
			logger: l,
			sev:    s,
		}
		f, n, err := create(l.dir, l.module, severityName[s])
		if err != nil {
			return err
		}
		sb.file = f
...
func (lb logBridge) Write(logging *loggingT, b []byte) (n int, err error) {
	var (
		file = "???"
		line = 1
		text string
...
func V(logging *loggingT, level Level) Verbose {
...
func Output(logging *loggingT, level int, depth int, format string, args ...interface{}) {
	...
	if l > logging.outputLevel {
		logging.printDepth(l, depth, format, args...)
	}
}
