...
func ParallelRequestOperatePsSpace(space *entity.Space, operation string) (e_str string) {
	// TODO:
	if operation == "create" {
		for _, partition := range space.Partitions {
			for _, replica := range partition.Replicas {
				createSpaceReq := entity.CreateSpaceRequest{CurPartition: partition, Space: *space}
				createSpaceReq.CurPartition = partition
				...
				if err != nil || resp.Status.Code != 0 {
					...
					return "query ps create space error"
				}
			}
		}
...
func CreateSpaceImpl(ctx context.Context, etcdCli *EtcdCli, space *entity.Space, 
	...
	if e_str = ParallelRequestOperatePsSpace(space, "create"); len(e_str) > 0 {
		return e_str
	}
	...
	if e := etcdCli.Put(ctx, etcdKey, spaceJson); e != nil {
		...
		log.Error(e_str)
		if e_str = ParallelRequestOperatePsSpace(space, "delete"); len(e_str) > 0 {
			return e_str
		}
		return e_str
	}
		...
		for _, space_info := range busySpaceList.NameToSpace {
			for _, partition_info := range space_info.Partitions {
				for _, rep_info := range partition_info.Replicas {
					del_ps_url := fmt.Sprintf("http://%s:%d/PsService/DeleteSpace", rep_info.PsIP, rep_info.PsPort)
					resp := entity.DeleteSpaceResponse{}
					resp_str, err := httputil.PostRequest(del_ps_url, nil, req_json, &resp)
					fmt.Printf("del_ps_url:%s, req_json:%s, resp_json:%s\n", del_ps_url, req_json, resp_str)
					if err != nil || resp.Status.Code != 0 {
						log.Errorf("query Ps_id:%d delete space, error:%v, msg:%s, resp_str:%s",
							       rep_info.PsID, err, resp.Status.Msg, resp_str)
						failed_del_ps = append(failed_del_ps, rep_info)
						continue
					}
				log.Info("Ps ip:%s, port:%d space:%s, success delete", rep_info.PsID, 
						 rep_info.PsPort, req.SpaceName)
				}
			}
		}
		failed_del_router := []entity.RouterReplica{}
		router_keys, router_vals, _ := ca.etcdCli.PrefixScan(ctx, config.RouterNodePrefix)
		routerReplicaList := entity.RouterReplicaList{}
		routerReplicaList.DeserializeFromByte(router_keys, router_vals)
		for _, router := range routerReplicaList.KeyToPsNode {
			del_ps_url := fmt.Sprintf("http://%s:%d/space/delete_space", router.RouterIP, router.RouterPort)
			resp := entity.DeleteSpaceResponse{}
			resp_str, err := httputil.PostRequest(del_ps_url, nil, req_json, &resp)
			fmt.Printf("del_ps_url:%s, req_json:%s, resp_json:%s\n", del_ps_url, req_json, resp_str)
			if err != nil || resp.Status.Code != 0 {
				log.Errorf("query router_ip:%d, port:%d delete space, error:%v, msg:%s, resp_str:%s",
						   router.RouterIP, err, resp.Status.Msg, resp_str)
				failed_del_router = append(failed_del_router, router)
				continue
			}
			log.Info("router ip:%s, port:%d space:%s, success delete",
				     router.RouterIP, router.RouterPort, req.SpaceName)
		}
