...
int main(int argc, char* argv[]) {
  ...
  if (local_storager.PsNodeMata().HasSpace()) {
    ...
    if (RaftManager::GetInstance().StartRaftServer(root_path, grp, conf, port).size() > 0) {
      LOG(ERROR) << "restart Raft error, server not write, only read.";
    }
  }

  etcd_cli.RegisterNodeWithKeepAlive(node_K, node_V, toml_conf_.ps.hearbeat_interval_s);
